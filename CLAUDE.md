...
Когда пользователь явно просит выполнить задачу через оркестрацию ИЛИ задача имеет сложность 4+:
1. Создай `.brain/plan.yaml` (формат выше)
2. Запусти `bash runner.sh .brain/plan.yaml` (runner сам сгенерирует промпты воркеров в `.brain/prompts/`, спавнит воркеров, будет ждать сигналы, запустит checkpoint и reviewer)
3. Дождись завершения runner.sh
4. Покажи результат пользователю

**Никогда не делай задачу сам если запрошена оркестрация.** Даже если задача простая — если пользователь сказал "через оркестрацию" / "через runner" / "запусти воркеров" → plan.yaml + runner.sh. Без исключений.
